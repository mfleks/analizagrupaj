...
```{r}
ggplot(rowery, aes(marital_status, fill=marital_status))+geom_bar(position="dodge")+facet_wrap(~region)+theme(legend.position='none')+labs(x="Stan cywilny", y="Ilość")+ggtitle("Ilość osób o poszczególnym stanie cywilnym w zależności od regionu")
```
...
1. Średnie dochody istotnie różnią się według analizowanego regionu,
2. Odsetek osób posiadających własny dom bądź mieszkanie istotnie różni się w zależności od rodzaju wykonywanej pracy,
3. Średnia liczba posiadanych samochodów istotnie różni się w zależności od posiadania roweru,
4. Poziom dochodów istotnie różni się w zależności od poziomu wykształcenia,
5. Poziom dzietności istotnie różni się w zależności od stanu cywilnego.

```{r}
ggbetweenstats(rowery, region, income)+ggtitle("Średni dochód w zależności od regionu")+labs(x="Region", y="Dochód")
```
Dla tych dwóch zmiennych zastosowano polecenie ggbetweenstats, adekwatne dla porównywania zmiennej jakościowej z ilościową. Na podstawie wartości p-value można stwierdzić, źe dochód istotnie różni się w zależności od regionu. Nie ma zatem podstaw, aby odrzucić pierwszą hipotezę badawczą.
...
```
Według wartości p-value można stwierdzić, że średnia liczba posiadanych samochodów jest istotnie niższa wśrod osób posiadających rower. Nie ma zatem podstaw, aby odrzucić trzecią hipotezę badawczą.
