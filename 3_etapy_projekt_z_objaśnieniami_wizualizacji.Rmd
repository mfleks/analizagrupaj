...
#Na rysunkach została przedstawiona wartość średnich dochodów według wieku kobiet i mężczyzn. W przypadku kobiet najwyższy dochód wynosił 85000 i był on osiągany w wieku 52 lat. Najniższy dochód wynosił około 10000 w wieku 68 lat. W przypadku meżczyzn najwyższy osiągany dochód wyniósł około 100000 w wieku 65 lat i najniższy około 13000 w wieku 25 lat. onnacza to, że w przypadku mężczyzn występuje większy rozstęp (różnica) średniego dochodu ale także jego większa średnia wartość.
```
...
#na ponizszym wykresie widać częstotliwość występowania obserwacji o określonym wieku i poziomie dochodów. Można zauważyć, że najwięcej obserwacji występuje w wieku około 25-45 lat z dochodem na poziomie od 2500 do 7500.
```
...
#Na poniższym wykresie pokazany jest stosunek liczby posiadanych samochodów w gospodarstwie domowym ze względu na region z którego pochodzą. Rzuca sie w oczy fakt, że w Ameryce Północnej najwięcej osób posiada co najmniej jeden samochód. Natomiast w Europie najwięcej gospodarstw domowych nie posiada w ogóle samochodu, ponieważ około 120, z jednym samochodem jest około 85 gospodarstw, z dwoma ok. 65, natomiast z trzema i czterema jest około 13-14 takich gospodarstw. Jeśli chodzi o Pacyfik to jest tam najmniejszy odsetek gospodarstw bez samochodu, ponieważ tylko około 20. Ten region charakteryzyje się najmniejszą dysproporcją jeżeli chodzi o liczbę samochodów, ponieważ liczba gospodarstw waha sie od ok. 20 z czterema samochodami do ok. 65 z czterema samochodami.
```
...
# Wykres pokazuje zagęszczenie obserwacji w zależności od wieku z podziałem na regiony. Najwieksze zageszczenie osób dla wszystkich obserwacji charakteryzują Europę i odnoszą się do osób w wieku od  wieku od 28 do 43 lat i wynosiło od od 0,02 do ok. 0,042. Dla osób w wieku od 43 do 63 lat, a także dla osób powyżej 72 roku życia najiększym zagęszczeniem obserwacji cieszyła sie Ameryka Północna. Wynosiło ono od ok. 0,012 do 0,033 w wieku 43-63 lata, a także od ok. 0,003 do 0,004 w wieku 72+. Dla pozostałej grupy wiekowej największym zagęszczeniem obserwacji charakteryzował sie Pacyfik i wynosiło ono ok. 0,004 do 0,012.
```
...
# Na wykresie pon9żej można dochody poszczególnych osób w zależności od rodzaju zawodów. najmniej zarabiaja pracownicy fizyczni. Ich dochód wynosił najcześciej ok. 1-2 tys.. Trochę więcej zarabiają pracownicy biurowi, gdyż ich dochody sięgają już kwoty 3-4 tys. jeszce więcej zarabiają wykwalifikowani pracownicy fizyczni, gdyz ich średni dochód sięga już kwoty średnio od 3 do 8 tys. Najwięcej zarabiają pracownicy profesjonalni, a także na stanowisku kierowniczym. Ich średni dochód siega kwoty od 4 do nawet 13 tys.
```
...
# Wykres pokazuje medianę dochodu, a także jego mksymalny, minimalny i średni dochód w zależności od płci. Większy sredni dochód charakteryzuje meżczyzn i wynosi on około 6 tysięcy. Średni dochód kobiet natomiast wynosi okolo 5,5 tysiąca. Mediana dochdów, a także jego maksymalna i minimalna wartość charakteryzuje się podobnymi wartościami wynoszącymi odpowiednio około: 6,5, 13 i 1 tys.
```
...
# Wykres przedstawia zależność posiadania samochodu, który zależy od tego czy badane odoby posiadają rower. Pierwszy wniosek jaki można wysunąć jest to, że osoby nieposiadające roweru mają więcej samochodów. Najwięcej (ok. 220) gospodarstw bez roweru posiada dwa samochody. Wówczas jeśli dane gospodarstwo posiada rower, to wtedy najwięcej gospodarstw domowych (ok. 155) posiada jeden samochód.
```
...
# Poniższy wykres przedstawia średni dochód według wykształcenia oraz rodzaju pracy. Kolorem żółtym oznaczono osoby zarabiające najmniej, czyli około 25000 natomiast kolorem czerwonym osoby zarabiające najwięcej (125000). Można zauważyć, że najmniej, czyli 25000 zarabiają osoby pracujące fizycznie, a także osoby na stanowisku urzędowym z wykształceniem wyższym. Około 50000 zarabiają pozostałe osoby pracujące na stanowisku urzędowym, a także profesjonaln pracownicy fizyczni. Około 75000 zarabiają pozostałe osoby wykwalifikowanie pracujący fizycznie, a także profesjonaliści z wykształceniem licealnym i licencjackim. Do tej grupy zalicza się także pracujących jako menedżerowie z wykształceniem wyższym. Około 100000 zarabiają prfesjonaliści z wykształceniem niepełnym wyższym i licencjackim. najwięcej, bo około 125000 zarabiają profesjonaliści z wykształceniem niepełnym licealnym ale także menedżerowie z wykształceniem licencjackim, licealnym i niepełnym wyższym.
```
...
# Na ostatnim wykresie przedstawiono medianę, maksimum, minimum i średnią wieku według stanu cywilnego. Średni wiek singla wynosi około 42 lata natomiast człowieka po ślubie 46 lat. Mediana wieku singla wynosi 40 lat natomiast osobę w związku małżeńskim 45 lat. Minimalny zaoserwowany wiek zarówno singla jak i osobę po ślubie wyniosła 25 lat. natomiast jeśli chodzi o ich maksymalny wiek to w przypadku singla najwyższy wiek wyniósł 69 lat, a w przypadku żonatego - 73 lata.
```
...
# Trzeci wykres przedstawia ile osob posiada rower ze wzgledu na wiek. Roweru nieposiada najwiecej osob w wieku 35-54 lata. natomiast takze najwiecej osob posiada rower w wieku 36-50 lat.
#czwarty wykres przedstawia kto jest wlascicielem domu w zaleznosci czy jest singlem czy w malzenstwie. 68% ludzi posiadajacych dom jest w zwiazku malzenskim, a 35% jest singlem. natomiast w 68% domu nie posiadaja single i 32% osob w zwiazkach malzenskich.
#Ostatni wykres przedstawia procentowy rozklad ludzi pracujących według wyksztalcenia. Najwiecej ludzi z wyksztalceniem licencjackim (32%) pracuje na stanowisku kierowniczym.Najmniej, bo 1% pracuje fizycznie. Najwięcej ludzi (41%) pracuje fizycznie z wyksztalceniem niepelnym srednim, a takze na stanowiskach biurowych (32%). Biorac pod uwage lidzi w wyksztalceniem wyzszym to najwiecej ludzi pracujena stanowisku kierowniczym(34%, a najmniej podczas fizycznej pracy (3%). biorac pod uwage ludzi z wyksztalceniem srednim pracujejako wykwalifikowany fizyczny (36%0, a najmniej na stanowisku biurowym (2%). jesli chodzi o ludzi  wyksztalceniem wyzszym nieskonczonym to najwiecej ludzi pracuje na stanowisku profesionalisty (30%), a najmniej na stanowisku kierowniczymv(2%).
```
